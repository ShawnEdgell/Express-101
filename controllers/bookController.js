...
exports.book_delete_post = asyncHandler(async (req, res, next) => {
  // Assuming you have the book ID in req.body.bookid
  const book = await Book.findOneAndDelete({ _id: req.body.bookid });

  if (book) {
    // Successfully deleted the book, redirect to book list or any other page you prefer
    res.redirect('/catalog/books');
  } else {
    // Handle the error if the book was not found or another error occurred
    const err = new Error('Book not found');
    err.status = 404;
    return next(err);
  }
